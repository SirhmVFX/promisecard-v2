      ...
      <section className="md:w-2/4 mx-auto p-6">
        ...
        <div className="py-8">
          {defaultgifts.map((gift) => (
            <div key={gift.id} className="flex gap-4 py-2 border-t-2">
              <div className={`p-2 ${gift.bg} rounded-lg`}>
                <Image src={gift.img} width={50} height={50} alt={gift.title} />
              </div>
              <div>
                <p className="text-black">{gift.title}</p>
                <p className="text-black">{gift.title}</p>
              </div>
            </div>
          ))}
